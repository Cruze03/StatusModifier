...
void (*StatusPrintClient_t)(CServerSideClientBase *pPlayer, bool verbose, CBufferString *clients) = nullptr;
...
void FASTCALL Hook_StatusPrintClient(CServerSideClientBase *pPlayer, bool verbose, CBufferString *clients)
{
	if (!pPlayer || pPlayer->IsFakeClient() || std::string(pPlayer->GetClientName()).empty())
	{
		StatusPrintClient_t(pPlayer, verbose, clients);
		return;
	}

	if (g_mExcludeSlots.find(pPlayer->GetPlayerSlot().Get()) != g_mExcludeSlots.end())
	{
		return;
	}
...
bool StatusModifier::Load(PluginId id, ISmmAPI *ismm, char *error, size_t maxlen, bool late)
{
	...
	SH_ADD_HOOK(INetworkServerService, StartupServer, g_pNetworkServerService, SH_MEMBER(this, &StatusModifier::Hook_StartupServer), true);
	SH_ADD_HOOK(IServerGameDLL, GameFrame, g_pSource2Server, SH_MEMBER(this, &StatusModifier::Hook_GameFrame), true);

	CModule libengine(engine);

	// search for "challenging" in engine2.dll or libengine2.so
#ifdef _WIN32
	const char *szSignature = "48 8B C4 55 48 8D A8 ? ? ? ? 48 81 EC ? ? ? ? 48 89 58 ? 49 8B D8";
#else
	const char *szSignature = "55 48 89 E5 41 57 41 56 4C 8D 3D ? ? ? ? 41 55 41 54 49 89 D4";
#endif

	StatusPrintClient_t = libengine.FindPattern(szSignature).RCast<decltype(StatusPrintClient_t)>();
	if (!StatusPrintClient_t)
	{
		ErrorLog("[%s] Failed to find function to get StatusPrint of client", g_PLAPI->GetLogTag());
		return false;
	}
	else
	{
...
bool StatusModifier::Unload(char *error, size_t maxlen)
{
	...
	SH_REMOVE_HOOK(INetworkServerService, StartupServer, g_pNetworkServerService, SH_MEMBER(this, &StatusModifier::Hook_StartupServer), true);
	SH_REMOVE_HOOK(IServerGameDLL, GameFrame, g_pSource2Server, SH_MEMBER(this, &StatusModifier::Hook_GameFrame), true);
...
void StatusModifier::Hook_StartupServer(const GameSessionConfiguration_t &config, ISource2WorldSession *pSession, const char *pszMapName)
{
	g_pEntitySystem = GameEntitySystem();
	g_bHasTicked = false;
...
void StatusModifier::Hook_GameFrame(bool simulating, bool bFirstTick, bool bLastTick)
{
	/**
	 * simulating:
	 * ***********
...
CON_COMMAND_F(mm_excludeslot, "Exclude slot from status", FCVAR_SERVER_CAN_EXECUTE)
{
...
CON_COMMAND_F(mm_removeexcludeslot, "Remove exclusion from status", FCVAR_SERVER_CAN_EXECUTE)
{
	if (context.GetPlayerSlot() != -1)
		return;
...
CON_COMMAND_F(mm_listexcludeslots, "List exclusion from status", FCVAR_SERVER_CAN_EXECUTE)
{
	if (context.GetPlayerSlot() != -1)
		return;
...
///////////////////////////////////////
const char *StatusModifier::GetLicense()
{
	return "GPL";
}

const char *StatusModifier::GetVersion()
{
	return "1.0.0";
}

const char *StatusModifier::GetDate()
{
	return __DATE__;
}

const char *StatusModifier::GetLogTag()
{
	return "StatusModifier";
}

const char *StatusModifier::GetAuthor()
{
	return "Cruze";
}
...
{
	return "Remove players you don't want to display in client console `status` command.";
}

const char *StatusModifier::GetName()
{
	return "Status Modifier";
}

const char *StatusModifier::GetURL()
{
